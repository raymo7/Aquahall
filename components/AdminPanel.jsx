                ...
                <div className="space-y-3">
                  {bookings.map((b) => (
                    <div key={b.id} className="rounded-2xl p-4 font-body text-sm" style={{ background: 'var(--cream-100)' }}>
                      ...
                      <p>{(b.services || []).map(serviceName).join(', ')} — {b.vehicle_type}{b.vehicle_model ? ` · ${b.vehicle_model}` : ''}</p>
                      <p>{b.booking_date} at {b.booking_time}</p>
                      <p><strong>Payment:</strong> {b.payment_method === 'advance' ? 'Advance' : 'Onsite'}</p>
                      <p style={{ color: 'var(--ink-muted)' }}>{b.address}</p>
                      {b.notes && <p style={{ color: 'var(--ink-muted)', fontStyle: 'italic' }}>"{b.notes}"</p>}
                      {!b.paid && (
                        <button onClick={() => markPaid(b.id)} disabled={loading} className="btn-ghost-teal mt-3 text-xs">Mark payment verified</button>
                      )}
                    </div>
                  ))}
                </div>
