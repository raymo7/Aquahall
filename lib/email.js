...
function bookingSummaryHtml(booking) {
  ...
  return `
    <table style="width:100%;border-collapse:collapse;font-size:14px;color:#241F1A;">
      ...
      <tr><td style="padding:10px 0 0;color:#5B5347;">Estimated service total</td><td style="padding:10px 0 0;"><strong style="color:#C85A2E;font-size:16px;">₹${booking.amount}${(booking.alacarte || []).includes('waterspot') ? '+' : ''}</strong></td></tr>
      ${(booking.alacarte || []).includes('waterspot') ? '<tr><td style="padding:4px 0;color:#5B5347;">Waterspot removal</td><td style="padding:4px 0;">Subject to affected area</td></tr>' : ''}
    </table>`;
}
