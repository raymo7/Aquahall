  ...
  <div style="background:#F8EED2;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #DCEEEC;">
      ...
      <div style="background:#F8EED2;padding:16px 28px;font-size:12px;color:#5B5347;">
        Aqua Haul · Doorstep vehicle care · 8 AM–9 PM daily<br/>
        aquahaul360@gmail.com
      </div>
    </div>
  </div>`;
...
function bookingSummaryHtml(booking) {
  ...
  return `
    <table style="width:100%;border-collapse:collapse;font-size:14px;color:#241F1A;">
      ...
      ${booking.group_offer ? '<tr><td style="padding:4px 0;color:#5B5347;">Group offer</td><td style="padding:4px 0;">10% off · same location</td></tr>' : ''}
      ...
      ${booking.distance_from_base_km != null ? `<tr><td style="padding:4px 0;color:#5B5347;">Distance</td><td style="padding:4px 0;">${escapeHtml(booking.distance_from_base_km)} km from Aqua Haul Base Station</td></tr>` : ''}
      ...
      ${booking.notes ? `<tr><td style="padding:4px 0;color:#5B5347;">Notes</td><td style="padding:4px 0;">${escapeHtml(booking.notes)}</td></tr>` : ''}
...
async function sendOneEmail(payload, label, recipient) {
  ...
  if (error) {
    ...
    console.error(`${label} failed for ${recipient}:`, error);
    return { recipient, ok: false, error: message };
  }

  return { recipient, ok: true, data };
}

async function sendOwnerEmails(payload, label) {
  if (!process.env.RESEND_API_KEY) {
    throw new Error('RESEND_API_KEY is not configured.');
  }

  if (!OWNER_EMAILS.length) {
    throw new Error('OWNER_EMAILS has no valid recipient addresses.');
  }

  const results = await Promise.all(
    OWNER_EMAILS.map((recipient) =>
      sendOneEmail(payload, label, recipient),
    ),
  );
  ...
  if (failures.length) {
    console.warn(
      `${label}: ${successes.length}/${results.length} owner emails delivered.`,
  ...
  // Do not make a working recipient fail because another owner address has a
  // Resend/domain/delivery problem. Throw only when nobody received the email.
  if (!successes.length) {
    throw new Error(
      failures.map((failure) => `${failure.recipient}: ${failure.error}`).join(' | '),
    );
  }
