...
export const HEAVY_VEHICLE_PRICE = 1000;
...
export function priceForPackage(vehicleType) { return categoryForVehicle(vehicleType) === 'heavy' ? HEAVY_VEHICLE_PRICE : (VEHICLE_PACKAGE_PRICES[vehicleType]?.complete ?? 800); }
...
export function resolveBooking({ vehicleType, vehicles, serviceType = 'complete', alacarte = [] }) {
  ...
  const vehicleCategories = vehicleList.map((vehicle) => categoryForVehicle(vehicle.type));
  const hasHeavyVehicle = vehicleCategories.includes('heavy');
  const allHeavyVehicles = vehicleCategories.every((category) => category === 'heavy');

  const category = allHeavyVehicles
    ? 'heavy'
    : hasHeavyVehicle
      ? 'mixed'
      : categoryForVehicle(primaryType);
  ...
  // A single heavy vehicle keeps the dedicated Heavy Vehicle Wash booking type.
  if (vehicleList.length === 1 && allHeavyVehicles) {
    return {
      category: 'heavy',
      packageId: null,
      serviceType: 'heavy',
      ...
      amount: HEAVY_VEHICLE_PRICE + fixedExtras.reduce((sum, id) => sum + addOnPrice(id), 0),
      variablePricing,
      vehicleCount: 1,
    };
  }

  // Vehicle Care Visit is cars-only. The UI prevents heavy vehicles from being
  // selected for it; this guard keeps totals safe if invalid data is submitted.
  ...
  const base = effectiveServiceType === 'vehicle-care'
    ...
    : vehicleList.reduce(
        (sum, vehicle) =>
          sum +
          (categoryForVehicle(vehicle.type) === 'heavy'
            ? HEAVY_VEHICLE_PRICE
  ...
  return {
    ...
    services: [
      ...pkg.includes,
      ...(hasHeavyVehicle && effectiveServiceType === 'complete' ? ['heavy'] : []),
      ...cleanAlacarte,
    ],
    alacarte: cleanAlacarte,
