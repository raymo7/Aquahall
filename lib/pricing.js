...
export const CORE_SERVICES = [
  ...
  { id: 'underbody', name: 'Underbody Wash', price: 0, included: true, selectable: false },
  ...
  { id: 'acvent', name: 'AC Vent Steaming', price: 150, pricingType: 'fixed', selectable: true, animation: 'steam', description: 'Focused steam treatment around the AC vents.' },
  { id: 'interiorsteam', name: 'Interior Steaming', price: 150, pricingType: 'fixed', selectable: true, animation: 'steam', description: 'A refreshing steam pass across key interior surfaces.' },
  ...
  // Legacy non-selectable aliases retained for compatibility with old bookings.
...
export const PACKAGES = {
  complete: {
    ...
    includes: ['foam', 'underbody', 'interior'],
    description: 'Foam Wash + Underbody Wash + Interior Detailing at your doorstep.',
  },
  vehicleCare: {
    id: 'vehicle-care',
    name: 'Vehicle Care Visit',
    includes: ['foam', 'underbody', 'interior'],
    description: 'For vehicles left unused or owners away from home: a basic visual check, start-up, short run/drive where safe, Complete Care Wash and photo/video update.',
  },
};
...
export function resolveBooking({
  ...
  // The 3-car promotion is intentionally narrow:
  // 3+ CAR vehicles, Complete Care Wash, one booking / same location.
  // Heavy vehicles and Vehicle Care Visit are not discounted.
  const groupDiscountEligible =
    Boolean(groupOffer) &&
    vehicleList.length >= 3 &&
    !hasHeavyVehicle &&
    effectiveServiceType === 'complete';

  const groupDiscount = groupDiscountEligible
    ? Math.round(base * (GROUP_DISCOUNT_PERCENT / 100))
    : 0;

  const discountedBase = base - groupDiscount;
  ...
  const pkg =
    effectiveServiceType === 'vehicle-care' ? PACKAGES.vehicleCare : PACKAGES.complete;
  ...
  return {
    ...
    amount: discountedBase + extrasTotal,
    variablePricing,
    vehicleCount: vehicleList.length,
  };
}
